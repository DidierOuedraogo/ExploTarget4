    ...
    if len(st.session_state.current_scenario["corps_mineralises"]) > 0:
        ...
        with viz_tab2:
            ...
            for i, corps in enumerate(st.session_state.current_scenario["corps_mineralises"]):
                # Calcul des coordonnées pour un parallélépipède rectangle simplifié
                azimuth_rad = np.radians(corps["azimuth"])
                inclinaison_rad = np.radians(corps["inclinaison"])
                
                # Point central du corps
                x0, y0 = 0, 0
                z0 = corps["elevation_toit"] - corps["epaisseur"] / 2  # Élévation négative pour être sous terre
                
                # Dimensions
                dx = corps["puissance"] * np.sin(azimuth_rad)
                dy = corps["puissance"] * np.cos(azimuth_rad)
                dz = corps["epaisseur"]
                
                # Extension en profondeur
                dx_prof = corps["profondeur"] * np.sin(azimuth_rad + np.pi/2) * np.cos(inclinaison_rad)
                dy_prof = corps["profondeur"] * np.cos(azimuth_rad + np.pi/2) * np.cos(inclinaison_rad)
                dz_prof = corps["profondeur"] * np.sin(inclinaison_rad)
                
                # Création des coordonnées des sommets
                x = [x0 - dx/2, x0 + dx/2, x0 + dx/2, x0 - dx/2, x0 - dx/2, x0 + dx/2, x0 + dx/2, x0 - dx/2]
                y = [y0 - dy/2, y0 - dy/2, y0 + dy/2, y0 + dy/2, y0 - dy/2, y0 - dy/2, y0 + dy/2, y0 + dy/2]
                z = [z0 - dz/2, z0 - dz/2, z0 - dz/2, z0 - dz/2, z0 + dz/2, z0 + dz/2, z0 + dz/2, z0 + dz/2]
                
                # Ajout de l'extension en profondeur
                for j in range(8):
                    x[j] += dx_prof * (j % 4 >= 2)  # Ajout pour les points arrières
                    y[j] += dy_prof * (j % 4 >= 2)  # Ajout pour les points arrières
                    z[j] -= dz_prof * (j % 4 >= 2)  # Soustraction pour l'extension en profondeur
                
                # Indices des faces (triangulation)
                i_indices = [0, 0, 0, 1, 1, 2, 2, 3, 4, 4, 4, 5, 5, 6, 6, 7, 0, 1, 2, 3, 4, 5, 6, 7]
                j_indices = [1, 3, 4, 2, 5, 3, 6, 0, 5, 7, 0, 6, 1, 7, 2, 4, 4, 5, 6, 7, 0, 1, 2, 3]
                k_indices = [2, 7, 5, 3, 6, 0, 7, 7, 6, 6, 3, 7, 2, 3, 3, 0, 3, 2, 7, 0, 7, 6, 3, 2]
                
                # Couleur basée sur l'indice (pour différencier les corps)
                colors = px.colors.qualitative.Plotly
                color = colors[i % len(colors)]
                
                # Ajout du corps minéralisé à la figure
                fig.add_trace(go.Mesh3d(
                    ...
                    f"Tonnage: {corps['puissance'] * corps['epaisseur'] * corps['profondeur'] * corps['densite'] * facteur_confiance:,.0f} t<br>" +
                    "<extra></extra>"
                ))
                
                # Ajout d'une représentation de la direction du corps
                fig.add_trace(go.Scatter3d(
                    x=[x0, x0 + 50 * np.sin(azimuth_rad)],
                    y=[y0, y0 + 50 * np.cos(azimuth_rad)],
                    z=[z0, z0],
                    mode='lines',
            ...
            st.caption("Cette visualisation 3D simplifiée montre les corps minéralisés selon leur orientation et dimensions. La surface verte représente le niveau du sol (z=0), et tous les corps minéralisés sont situés sous terre (z<0).")
    ...
    if len(st.session_state.current_scenario["corps_mineralises"]) == 0:
        ...
        if corps_a_forer:
            ...
            for corps in corps_selectionnes:
                # Surface approximative du corps
                surface_corps = corps["puissance"] * corps["profondeur"]
            ...
            for corps_idx, corps in enumerate(corps_selectionnes):
                ...
                # Point central du corps
                x0, y0 = 0, 0
                z0 = corps["elevation_toit"] - corps["epaisseur"] / 2
                ...
                # Ajout du corps minéralisé à la figure (semi-transparent)
                x = [x0 - dx/2, x0 + dx/2, x0 + dx/2, x0 - dx/2, x0 - dx/2, x0 + dx/2, x0 + dx/2, x0 - dx/2]
                y = [y0 - dy/2, y0 - dy/2, y0 + dy/2, y0 + dy/2, y0 - dy/2, y0 - dy/2, y0 + dy/2, y0 + dy/2]
                z = [z0 - dz/2, z0 - dz/2, z0 - dz/2, z0 - dz/2, z0 + dz/2, z0 + dz/2, z0 + dz/2, z0 + dz/2]
                ...
                fig.add_trace(go.Mesh3d(
                    ...
                    color=color,
                    opacity=0.2,
                    hovertemplate=f"<b>{corps['nom']}</b><extra></extra>"
                ))
                ...
                # Calculer les positions des forages pour former une grille
                x_start = x0 - dx/2 - dx_prof/2
                y_start = y0 - dy/2 - dy_prof/2
                
                for ix in range(int(nb_forages_x)):
                    for iy in range(int(nb_forages_y)):
                        # Position de départ du forage (au niveau du sol z=0)
                        x_forage = x_start + (ix + 0.5) * (dx + dx_prof) / nb_forages_x
                        y_forage = y_start + (iy + 0.5) * (dy + dy_prof) / nb_forages_y
                        z_forage = 0  # Surface (altitude zéro)
                ...
                for ix in range(0, int(nb_forages_x_detail), step_x):
                    for iy in range(0, int(nb_forages_y_detail), step_y):
                        ...
                        # Position de départ du forage (au niveau du sol z=0)
                        x_forage = x_start + (ix + 0.5) * (dx + dx_prof) / nb_forages_x_detail
                        y_forage = y_start + (iy + 0.5) * (dy + dy_prof) / nb_forages_y_detail
                        z_forage = 0  # Surface
            ...
            st.plotly_chart(fig, use_container_width=True)
            st.caption(f"""
            Cette visualisation 3D montre le plan de forage proposé. Les forages initiaux (rouges) sont complétés par des forages détaillés (bleus).
            La surface verte représente le niveau du sol (z=0). Tous les forages ont un azimut de {azimuth_forage}°, une inclinaison de {inclinaison_forage}° 
...
# Page du guide utilisateur
    ...
    **Preliminary Explo Target Estimation** est un outil conçu pour aider les géologues et les professionnels de l'exploration minière à estimer rapidement les ressources minérales pendant les phases initiales d'exploration et à planifier efficacement les campagnes de forage.
    ...
    st.markdown('<div class="card">', unsafe_allow_html=True)
    st.markdown("""
    Les corps minéralisés sont les unités géologiques contenant la minéralisation. Pour définir un corps minéralisé:
    ...
    2. Dans la section **Ajouter un corps minéralisé**, renseignez les paramètres suivants:
       ...
       - **Puissance (m)**: Largeur/extension horizontale du corps
       - **Épaisseur (m)**: Épaisseur verticale moyenne
       - **Profondeur (m)**: Extension en profondeur
       ...
       - **Azimuth (°)**: Direction du corps (0° = Nord, 90° = Est)
       - **Inclinaison (°)**: Angle par rapport à l'horizontale
       - **Élévation du toit (m)**: Altitude du point le plus haut du corps (valeur négative pour être sous terre)
    
    3. Cliquez sur **Ajouter ce corps minéralisé**
    
    Vous pouvez ajouter plusieurs corps minéralisés à un même scénario pour représenter différentes zones d'intérêt ou différentes lithologies.
    """)
    
    # Exemple d'illustration pour les paramètres géométriques
    st.markdown("### Paramètres géométriques des corps minéralisés")
    st.image("https://via.placeholder.com/800x400.png?text=Illustration+des+parametres+geometriques", 
            caption="Illustration des paramètres géométriques d'un corps minéralisé")
    st.markdown('</div>', unsafe_allow_html=True)
    ...
    Les forages additionnels sont planifiés pour traverser perpendiculairement (ou presque) les corps minéralisés, en tenant compte de leur azimuth et inclinaison.
    """)
    
    st.markdown("### Stratégie de forage recommandée")
    st.markdown("""
    1. **Phase initiale**: Utiliser une maille large (100-200m) pour identifier et délimiter les zones minéralisées
    2. **Phase détaillée**: Resserrer la maille (25-50m) dans les zones d'intérêt pour améliorer la confiance dans l'estimation
    
    Les forages détaillés doivent être positionnés entre les forages initiaux pour maximiser la couverture et minimiser la redondance.
    """)
    st.markdown('</div>', unsafe_allow_html=True)
    ...
    - Utilisez des paramètres conservateurs pour la teneur et les dimensions du corps minéralisé
    - Tenez compte de la continuité géologique dans la définition des corps
    - Divisez les zones complexes en plusieurs corps minéralisés simples
    - Vérifiez que la densité utilisée correspond bien au type de mineralization
    
    ### Pour la planification de forage:
    
    - Adaptez l'espacement de la maille à la complexité géologique
    - Orientez les forages perpendiculairement aux corps pour minimiser la longueur nécessaire
    - Prévoyez quelques forages plus profonds pour tester les extensions en profondeur
    - Pour les corps à fort pendage, privilégiez les forages inclinés
    
    ### Pour la budgétisation:
    
    - Incluez une marge de contingence (10-15%)
    - N'oubliez pas les coûts annexes: logistique, hébergement, supervision
    - Prévoyez un budget pour des analyses complémentaires
    - Tenez compte des conditions d'accès au site dans l'estimation des coûts
    """)
    st.markdown('</div>', unsafe_allow_html=True)
